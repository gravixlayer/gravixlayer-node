...
export class Accelerators {
  ...
  async list(): Promise<Accelerator[]> {
    ...
    (this.client as any).baseURL = originalBaseURL.replace('/v1/inference', '/v1');
    ...
    try {
      ...
      if (Array.isArray(acceleratorsData)) {
        ...
        console.warn(`Unexpected response format: ${typeof acceleratorsData}, content:`, acceleratorsData);
        return [];
      }
